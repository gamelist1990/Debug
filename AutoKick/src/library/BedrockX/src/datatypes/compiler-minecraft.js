...
Write.maybeIncompleteArray = ['parametrizable', (compiler, { countType, type }) => compiler.wrapCode(`
  offset = ${compiler.callType('value.length', countType)}
  for (let i = 0; i < value.length; i++) offset = ${compiler.callType('value[i]', type)}
  return offset
`.trim())]
SizeOf.maybeIncompleteArray = ['parametrizable', (compiler, { countType, type }) => compiler.wrapCode(`
  let size = ${compiler.callType('value.length', countType)}
  for (let i = 0; i < value.length; i++) size += ${compiler.callType('value[i]', type)}
  return size
`.trim())]

Read.optionalOnRemaining = ['parametrizable', (compiler, { type }) => compiler.wrapCode(`
  if (offset >= buffer.length) return { value: undefined, size: 0 }
  return ${compiler.callType(type)}
`.trim())]
Write.optionalOnRemaining = ['parametrizable', (compiler, { type }) => compiler.wrapCode(`
  if (value === undefined) return offset
  return ${compiler.callType('value', type)}
`.trim())]
SizeOf.optionalOnRemaining = ['parametrizable', (compiler, { type }) => compiler.wrapCode(`
